# shamelessly taken from lintr
# - The code here has yet to be pushed into lintr on CRAN, but is required to
# parse files that contain non-R codeblocks, eg, ```{python} \n a = 1 ```
...
extract_r_source <- function(filename, lines) {
  ...
  if (length(starts) == 0 || length(ends) == 0) {
    # TODO: add this as return value in `lintr`
    return(character(0))
  }
...
get_knitr_pattern <- function(filename, lines) {
  pattern <- (
    ...
    lines, tolower(("knitr" %:::% "file_ext")(filename))
  )
  if (!is.null(pattern)) {
