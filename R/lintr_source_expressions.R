...
# Shamelessly taken from github lintr
...
get_source_expressions <- function(filename) {
  ...
  lint_error <- function(e) {
    ...
    if (is.na(message_info$line)) {
      ...
      line_location <- re_matches(
        source_file$content, rex(message_info$starting),
        locations = TRUE
      )
  ...
  expressions[[length(expressions) + 1L]] <-
    list(
      ...
      xml_parsed_content = if (!is.null(parsed_content)) {
...
get_source_file <- function(source_file, error = identity) {
  e <- tryCatch(
    source_file$parsed_content <- parse(
      text = source_file$content, srcfile = source_file, keep.source = TRUE
    ),
    error = error
  )
...
fix_column_numbers <- function(content) {
  ...
  for (i in to_change) {
    needs_adjustment <- which(
      content$line1 == content$line1[i] & content$col1 >= content$col1[i]
    )
...
# nolint end
# The number of spaces is so that the code is brought to the next 8-character
...
fix_tab_indentations <- function(source_file) {
  ...
  # source lines from .Rmd and other files are NA
  tab_cols <- tab_cols[!is.na(tab_cols)]
  tab_cols <- lapply(tab_cols, function(x) {
    if (x[[1L]] < 0L) {
      NA
    } else {
      x
    ...
    if (any(is_curr_line)) {
      ...
      for (j in seq_along(tab_cols[[i]])) {
        ...
        if (any(is_line_to_change)) {
          dat[is_line_to_change, 2L] <-
            dat[is_line_to_change, 2L] - line_tab_offsets[[j]]
        }
      }
    }
  }
...
tab_offsets <- function(tab_columns) {
  ...
  vapply(
    tab_columns - 1L,
    function(tab_idx) {
      # using a tab width of 8 characters
      offset <- 7L - (tab_idx + cum_offset) %% 8L
...
prev_with_parent <- function(pc, loc) {
  ...
  with_parent <- with_parent[
    order(
      with_parent$line1, with_parent$col1, with_parent$line2, with_parent$col2
    ),
  ]
